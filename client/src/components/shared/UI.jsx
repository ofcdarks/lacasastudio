...
// ── Design tokens accessible in JS ──
export const C = {
...
export function PBar({ current, target, color, h = 6 }) {
...
export function Badge({ text, color = C.red, v = "dot" }) {
...
export function Card({ children, style: s = {}, color, onClick, hov = false }) {
...
export function Btn({ children, onClick, vr = "primary", style: s = {}, disabled = false }) {
  const [h, setH] = useState(false);
  const base = {
  ...
  return (
    <button onClick={disabled ? undefined : onClick}
      ...
      style={{ ...base, ...vs[vr] }}>{children}</button>
  );
}
