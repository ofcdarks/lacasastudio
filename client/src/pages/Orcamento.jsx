...
export default function Orcamento() {
  ...
  return (
    <div className="page-enter">
      <Hdr title="Orçamento" sub="Controle receitas e despesas dos seus canais" action={<Btn onClick={() => setShowF(!showF)}>{showF ? "✕ Fechar" : "+ Novo Lançamento"}</Btn>} />
      ...
      <Card style={{ padding: 0 }}>
        {items.length === 0 ? (
          ...
          <div key={it.id} style={{ display: "grid", gridTemplateColumns: "auto 1fr 2fr 1fr 32px", gap: 12, alignItems: "center", padding: "12px 20px", borderBottom: i < items.length - 1 ? `1px solid ${C.border}` : "none" }}
            ...
            <span style={{ fontFamily: "var(--mono)", fontSize: 13, fontWeight: 600, textAlign: "right", color: it.type === "income" ? C.green : C.red }}>
              {it.type === "income" ? "+" : "-"} R$ {it.value.toLocaleString("pt-BR")}
            </span>
            <Btn vr="subtle" onClick={() => delItem(it.id)} style={{ fontSize: 10, color: C.red, opacity: 0.5, padding: "4px" }}>✕</Btn>
          </div>
        ))}
      </Card>
    </div>
