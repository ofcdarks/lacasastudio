      ...
      <div style={{display:"flex",flex:1,overflow:"hidden"}}>
        ...
        <div ref={box} style={{flex:1,position:"relative",cursor:resizing?({"nw":"nwse-resize","ne":"nesw-resize","sw":"nesw-resize","se":"nwse-resize","n":"ns-resize","s":"ns-resize","e":"ew-resize","w":"ew-resize"}[resizing.dir]||"move"):isPan?"grab":TOOLS[tool]?.c||"default",overflow:"hidden"}}>
          ...
          {textEditor&&<div style={{position:"absolute",left:Math.max(10,Math.min((textEditor.canvasX||100)*zoom+pan.x, (box.current?.clientWidth||800)-320)),top:Math.max(10,Math.min((textEditor.canvasY||100)*zoom+pan.y-30, (box.current?.clientHeight||600)-220)),zIndex:40,width:320}}>
            <div style={{background:C.bgCard,borderRadius:10,border:`1px solid ${C.blue}`,padding:12,boxShadow:"0 8px 30px rgba(0,0,0,.6)"}}>
              <div style={{fontSize:10,color:C.dim,marginBottom:6}}>Fonte: {FONTS[fontIdx].n} · {sizeKey} {boldV?"· Bold ":""}{ italicV?"· Itálico":""} — Esc cancelar</div>
              <textarea autoFocus value={textEditor.value} onChange={e=>setTextEditor(p=>({...p,value:e.target.value}))}
                onKeyDown={e=>{if(e.key==="Escape"){e.preventDefault();setTextEditor(null);}}}
                style={{width:"100%",background:"rgba(255,255,255,.06)",border:`1px solid ${C.border}`,borderRadius:6,padding:"10px",color,fontSize:Math.min(textEditor.fontSize||SIZES[sizeKey],32),fontFamily:FONTS[fontIdx].f,fontWeight:boldV?"bold":"normal",fontStyle:italicV?"italic":"normal",outline:"none",minHeight:70,resize:"vertical",lineHeight:1.4}}
                placeholder="Digite aqui..."/>
              <div style={{display:"flex",gap:6,justifyContent:"flex-end",marginTop:8}}>
                <button onClick={()=>setTextEditor(null)} style={{padding:"5px 12px",borderRadius:6,border:`1px solid ${C.border}`,background:"transparent",color:C.muted,cursor:"pointer",fontSize:11}}>Cancelar</button>
                <button onClick={confirmText} style={{padding:"5px 14px",borderRadius:6,border:"none",background:C.red,color:"#fff",cursor:"pointer",fontSize:11,fontWeight:600}}>OK</button>
              </div>
            </div>
          </div>}
...
Ex: Roteiro de reels sobre produtividade" style={{width:"100%",background:"rgba(255,255,255,.04)",border:`1px solid ${C.border}`,borderRadius:8,padding:"12px",color:C.text,fontSize:13,outline:"none",minHeight:100,resize:"vertical",marginBottom:16}}/>
        ...
        {!zenMode&&<div style={{width:220,minWidth:220,background:C.bgCard,borderLeft:`1px solid ${C.border}`,overflowY:"auto",padding:"14px"}}>
          ...
          <PS label="Centralizar"><div style={{display:"flex",gap:3}}><button onClick={()=>centerEl("h")} disabled={!selId} style={{flex:1,padding:"5px 0",borderRadius:6,border:"none",cursor:selId?"pointer":"not-allowed",fontSize:10,fontWeight:600,background:"rgba(255,255,255,.04)",color:selId?C.muted:C.dim,opacity:selId?1:.4}}>↔ Horizontal</button><button onClick={()=>centerEl("v")} disabled={!selId} style={{flex:1,padding:"5px 0",borderRadius:6,border:"none",cursor:selId?"pointer":"not-allowed",fontSize:10,fontWeight:600,background:"rgba(255,255,255,.04)",color:selId?C.muted:C.dim,opacity:selId?1:.4}}>↕ Vertical</button><button onClick={()=>centerEl("both")} disabled={!selId} style={{flex:1,padding:"5px 0",borderRadius:6,border:"none",cursor:selId?"pointer":"not-allowed",fontSize:10,fontWeight:600,background:"rgba(255,255,255,.04)",color:selId?C.muted:C.dim,opacity:selId?1:.4}}>⊞ Ambos</button></div></PS>
          <PS label="Desfazer / Refazer"><div style={{display:"flex",gap:3}}><button onClick={undo} disabled={hIdx<=0} style={{flex:1,padding:"6px 0",borderRadius:6,border:"none",cursor:hIdx>0?"pointer":"not-allowed",fontSize:11,fontWeight:600,background:hIdx>0?`${C.blue}15`:"rgba(255,255,255,.04)",color:hIdx>0?C.blue:C.dim}}>↩ Ctrl+Z</button><button onClick={redo} disabled={hIdx>=hist.length-1} style={{flex:1,padding:"6px 0",borderRadius:6,border:"none",cursor:hIdx<hist.length-1?"pointer":"not-allowed",fontSize:11,fontWeight:600,background:hIdx<hist.length-1?`${C.blue}15`:"rgba(255,255,255,.04)",color:hIdx<hist.length-1?C.blue:C.dim}}>↪ Ctrl+Y</button></div></PS>
          <PS label="Canvas background"><div style={{display:"flex",gap:4,flexWrap:"wrap"}}>{CANVAS_BGS.map(bg=><div key={bg} onClick={()=>setCanvasBg(bg)} style={{width:26,height:26,borderRadius:6,cursor:"pointer",background:bg,border:canvasBg===bg?"2px solid #3B82F6":`1px solid ${C.border}`}}/>)}</div></PS>
          <PS label="Grade"><div style={{display:"flex",alignItems:"center",gap:10,cursor:"pointer"}} onClick={()=>setShowGrid(p=>!p)}><div style={{width:38,height:22,borderRadius:11,background:showGrid?C.blue:C.border,position:"relative",transition:"all .2s"}}><div style={{width:18,height:18,borderRadius:9,background:"#fff",position:"absolute",top:2,left:showGrid?18:2,transition:"all .2s"}}/></div><span style={{fontSize:12,color:showGrid?C.text:C.dim}}>{showGrid?"Grade visível":"Grade oculta"}</span></div></PS>
