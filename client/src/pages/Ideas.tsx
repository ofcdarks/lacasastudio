...
export default function Ideas(){
  ...
  if(showList)return(
    <div className="page-enter" style={{maxWidth:960,margin:"0 auto"}}>
      <Hdr title="Banco de Ideias" sub="Whiteboard para brainstorm e planejamento" action={<Btn onClick={newB}>+ Novo Quadro</Btn>}/>
      <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(240px,1fr))",gap:16}}>
        ...
        {boards.map(b=><div key={b.id} style={{background:C.bgCard,borderRadius:14,border:`1px solid ${C.border}`,overflow:"hidden",cursor:"pointer",transition:"all .2s"}} onMouseEnter={e=>{e.currentTarget.style.borderColor=C.borderH;e.currentTarget.style.transform="translateY(-2px)";}} onMouseLeave={e=>{e.currentTarget.style.borderColor=C.border;e.currentTarget.style.transform="none";}}>
          ...
          <div style={{padding:"10px 14px",display:"flex",alignItems:"center",justifyContent:"space-between"}}><div onClick={()=>load(b)}><div style={{fontWeight:600,fontSize:13}}>{b.title}</div><div style={{fontSize:10,color:C.dim}}>{new Date(b.updatedAt||b.createdAt).toLocaleDateString("pt-BR")}</div></div><Btn vr="subtle" onClick={e=>{e.stopPropagation();delB(b.id);}} style={{color:C.red,fontSize:11}}>✕</Btn></div></div>)}
      </div></div>);
