...
export default function Templates() {
  ...
  const addT = async () => {
    ...
    try {
      const tpl = await templateApi.create({ ...nt, channelId: nt.channelId ? Number(nt.channelId) : null, structure: nt.structure.split(",").map(s => s.trim()).filter(Boolean) });
      setTpls(p => [tpl, ...p]);
  ...
  return (
    <div className="page-enter">
      <Hdr title="Templates de Série" sub="Crie templates reutilizáveis" action={<Btn onClick={() => setShowF(!showF)}>{showF ? "✕" : "+ Novo Template"}</Btn>} />
      {showF && (
        <Card style={{ marginBottom: 20, borderColor: `${C.purple}30` }}>
          ...
          <div style={{ marginBottom: 10 }}><Label t="Descrição" /><Input value={nt.desc} onChange={e => setNt(p => ({ ...p, desc: e.target.value }))} /></div>
          <div><Label t="Estrutura (vírgula)" /><Input placeholder="Intro (15s), Conteúdo (5min), CTA" value={nt.structure} onChange={e => setNt(p => ({ ...p, structure: e.target.value }))} /></div>
        </Card>
      )}
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(340px, 1fr))", gap: 14 }}>
        {tpls.map(t => {
          ...
          return (
            <Card key={t.id} hov color={t.color || ch?.color}>
              <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 12 }}>
                <div style={{ width: 42, height: 42, borderRadius: 11, background: `${t.color || C.blue}12`, border: `1px solid ${t.color || C.blue}30`, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 18 }}>◆</div>
                <div style={{ flex: 1 }}><div style={{ fontWeight: 700, fontSize: 15 }}>{t.name}</div><div style={{ fontSize: 12, color: ch?.color || C.muted }}>{ch?.name || "Global"}</div></div>
                {t.episodes > 0 && <Badge text={`${t.episodes} ep.`} color={t.color} v="tag" />}
              </div>
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
                  {structure.map((step, i) => (
                    <div key={i} style={{ display: "flex", alignItems: "center", gap: 10, padding: "8px 12px", background: "rgba(255,255,255,0.02)", borderRadius: 8, borderLeft: `3px solid ${(t.color || C.blue)}40` }}>
                      <span style={{ fontFamily: "var(--mono)", fontSize: 10, color: t.color, fontWeight: 700, width: 18 }}>{i+1}</span>
