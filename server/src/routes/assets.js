...
router.post("/", async (req, res, next) => {
  try {
    const { name, type, format, size, channelId, tags } = req.body;
    if (!name) return res.status(400).json({ error: "Nome obrigatório" });
    const asset = await prisma.asset.create({
      data: { name, type: type || "thumbnail", format: format || "", size: size || "", channelId: channelId ? Number(channelId) : null, tags: Array.isArray(tags) ? tags.join(",") : (tags || "") },
